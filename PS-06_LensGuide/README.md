...
`01_YOUR_DATA_MODEL.pdf` lists all 13 tables you have been given — 12 that this statement lives in, plus reference tables — with every field.
...
- **Run `data/queries/starter_queries.sql`.** 5 queries, each with a comment explaining what it shows, all of them running as-is against `data/PS-06.db`:

```bash
sqlite3 data/PS-06.db < data/queries/starter_queries.sql
```

That is 12,289 rows of real, queryable travel data. It is fully synthetic — no real people, no personal data — so it is safe to commit to a public repository.
...
| `data/PS-06.db` | SQLite, indexed, only your tables. No setup. |
